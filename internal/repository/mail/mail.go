...
type Post interface {
	SendMsg(msg *entity.Message) error
	Start()
	Shutdown()
}
...
type MailServer struct {
	...
	wg           sync.WaitGroup
	conn         *gomail.Dialer
	contexts     []entity.WorkerContext
}

// При передаче 0 в параметры будут выставлены default значения
func New(cfg config.Mail, logger *zap.Logger, WriteTimeout time.Duration, MsgBucketSize, Workers int) Post {
	if WriteTimeout == 0 {
	...
	return &MailServer{
		...
		workers:      Workers,
		contexts:     make([]entity.WorkerContext, Workers),
		wg:           sync.WaitGroup{},
		Logger:       logger,
	}
}
...
func (s *MailServer) SendMsg(msg *entity.Message) error {
