...
// Сборка письма
func letterAssembly(data *entity.Message) (*gomail.Message, error) {
	...
	case ReminderType:
		...
		fields, ok := data.Value.(entity.ReminderMsg)
		if !ok {
			return nil, ErrWrongMsgType
		}
		body = fmt.Sprintf(reminderTemplate,
			fields.Organization,
			fields.Service,
			fields.SessionStart.Format("15:06")+"-"+fields.SessionEnd.Format("15:06"),
		)
		if data.IsAttach {
...
func icsCreate(msg entity.ReminderMsg) string {
